...
token_chinese = BertTokenizer.from_pretrained('/root/autodl-tmp/bert-base-chinese')
token_uncased = BertTokenizer.from_pretrained('/root/autodl-tmp/bert-base-uncased')
...
def collate_fn_gossipcop_LLM(data):
    """ In Weibo dataset
        if not self.with_ambiguity:
            return (content, img_GT, label, 0), (GT_path)
        else:
    ...
    image = [i[1] for i in data]
    image_aug = [i[1] for i in data]
    labels = [i[2] for i in data]
    category = [i[3] for i in data]
    GT_path = [i[4] for i in data]
    token_data = token_chinese.batch_encode_plus(batch_text_or_text_pairs=sents,
                                   truncation=True,
                                   padding='max_length',
                                   max_length=word_token_length,
...
def main(args):
    ...
    elif setting['train_dataname']=='Gossipcop_LLM':
        from data.Gossipcop_LLM_dataset import Gossipcop_LLM_dataset
        train_dataset = Gossipcop_LLM_dataset(is_train=True)
        train_loader = DataLoader(train_dataset, batch_size=custom_batch_size, shuffle=shuffle,collate_fn=collate_fn_gossipcop_LLM,
                                           num_workers=0, sampler=train_sampler, drop_last=True,
                                           pin_memory=True)
        setting['thresh'] = 0.5
    else:
    ...
    elif setting['val_dataname']=='Gossipcop_LLM':
        from data.Gossipcop_LLM_dataset import Gossipcop_LLM_dataset
        validate_dataset = Gossipcop_LLM_dataset(is_train=False)
        validate_loader = DataLoader(validate_dataset, batch_size=custom_batch_size, shuffle=False,collate_fn=collate_fn_gossipcop_LLM,
        ...
        with torch.no_grad():
            ...
            if setting['get_MLP_score']>0:
                ...
                if validate_acc > best_validate_acc:
                    ...
                    output_dir = os.path.join(args.output_file,setting['train_dataname'])
                    if not os.path.exists(output_dir):
                        os.makedirs(output_dir)
                    best_validate_dir = "{}_{}{}_{}.pkl".format(str(epoch + 1),str(datetime.datetime.now().month),str(datetime.datetime.now().day),
                                                                      int(best_validate_acc*100))
                    output_path = os.path.join(output_dir, best_validate_dir)
...
def evaluate(validate_loader, model, criterion, progbar=None, setting={}):
    ...
    df_dir = '/root/autodl-tmp/groupshare/mae-main/example/'
    if not os.path.exists(df_dir):
        os.makedirs(df_dir)
    pandas_file = df_dir + f'{dataset_name}_experiment.xlsx'
